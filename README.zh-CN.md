...
- **C8** 保序回归的混淆移除能力是**依赖数据集**的：在 HS2 上把跨评判器
  ...
  `R̂*_CA`（图 11）。
- **C9** 逐子样本重拟合保序回归的样本复杂度在合成数据和真实数据上**都比
  CLT 更陡**——HS2 上斜率 −0.750 (R² = 0.968)，2-Gaussian toy（解析
  `R* = 0.159`）上斜率 −0.626。这一*效应*（校准器重拟合在 i.i.d. 抽样之外
  叠加了方差结构）是可复现的；具体斜率取决于数据。早期使用固定校准器的版本
  trivially 验证了 CLT (−0.500)，该实验已修正（图 12, 图 14）。
- **C10** HS2 ↔ UF 6 评判器排名一致性：Spearman 0.77 (N=6)。两个
  数据集在协变量漂移和概念漂移上都有差异（人类多数 vs. GPT-4
  分数差金标），所以这是一项排名稳定性检查，*不是*一次
  干净的 OOD 迁移测试（图 13）。
- **C11** 零置换控制实验：打乱 HS2 金标后得到
  `R̂*_iso ≈ 0.49` = 类别先验 `min(p, 1−p)`，且插入法 `R̂*` 对金标不变。
  4/6 评判器（Llama-3 / Granite / Mistral / Qwen）的真实
  `R̂*_iso` 落在零分布 95% 置信区间**外**——是真正的信号；OLMo 和
  Falcon 落在零置信区间**内**——它们的高 `R̂*` 在统计意义上与随机标签
  无法区分，直接放大了容量混淆的叙事（图 15）。
- **C12** 该混淆**对金标过滤严格度不变**。扫描 UF 分数差阈值
  `τ ∈ {2, 3, 4, 5}`（样本量从 999 收缩到 307），
  `r(acc, R̂*_iso)` 始终在 `[−0.999, −1.000]` 区间，Fisher-z 置信区间
  上界 ≤ −0.988。该效应并非 `τ = 2` 默认切分造成的 artifact（图 16）。
- **C13** **朴素众包聚合无法取代 `R̂*_CA`。**
  对每个评判器保序回归输出做中位数聚合，得到每个数据集一个单一
  共识 `R̂*`（HS2: 0.445, UF: 0.299；drift 0.146）。由于中位数无法消除
  *系统性*的容量偏差（弱评判器的 `c_k` 塌缩到 0.5），该共识复现的是
  中位容量评判器的估计值，而非数据侧的不变量
  （[Dawid–Skene 1979](https://www.jstor.org/stable/2346806)、
  [Raykar et al. 2010](https://www.jmlr.org/papers/v11/raykar10a.html)
  这条脉络）。这就确立了**第 1 年间隔匹配必须击败的
  minimum-viable-aggregator baseline**（图 17）。
- **C14** **FCI-Risk：部分概念验证 + 建设性反例。** 为了取代
  P0 V4 中那个同义反复的 `R̂*_CA` 公式（P14），我们实现了一个
  前向校正保序（Forward-Corrected Isotonic）估计器，灵感来自
  [Patrini et al. 2017](https://openaccess.thecvf.com/content_cvpr_2017/html/Patrini_Making_Deep_Neural_CVPR_2017_paper.html)
  的前向校正和 [Liu & Tao 2015](https://ieeexplore.ieee.org/document/7159100)
  的锚点方法。该仿射校正被放在 `min(·)` *内部*，因此该估计器
  在代数上不可约化为 `R̂*_iso`。**在 UF 上**，FCI-q01/q99 把
  跨评判器标准差压缩了 43%（0.128 → 0.073），并把 HS2↔UF
  漂移降低了 14%（0.134 → 0.114）。**在 HS2 上**，只有 OLMo
  存在可测量的容量截断（拉伸因子 1.87）；FCI 把 OLMo 向下
  重新缩放，而其余 5 个评判器几乎保持不变，因此跨评判器
  跨度反而从 0.031 *扩大*到 0.061。这正是第 1 年有限样本
  定理必须处理的结构性缺口——当截断在评判器之间异质时，
  逐评判器锚点的估计器无法保持稳健。整体上该实验属于
  **部分通过（partial PASS）**：在某些数据集上存在 CA 路线
  可行的方向性证据，同时给出一个具体的建设性反例，说明需要
  定理而非启发式打补丁（图 18）。
...
| C9 (synth) | 重拟合保序回归斜率在合成 toy 上同样比 CLT 更陡 | [图 14](experiments/P0-synthetic-confounding/fig_synthetic_sample_complexity.png) + [results_v5_sample_complexity.json](experiments/P0-synthetic-confounding/results_v5_sample_complexity.json) | 合成斜率 −0.626（HS2 −0.750）；`\|syn − hs2\| = 0.124 < 0.15` 阈值；R² = 0.951；证实校准器重拟合在 CLT 之外叠加了偏差-方差耦合，与数据来源无关 |
| C11 | 弱评判器（Falcon、OLMo）产生的信号与随机标签无法区分 | [图 15](experiments/P11-null-sanity/fig_null_sanity.png) + [stats.json](experiments/P11-null-sanity/stats.json) | 200 次金标置换：零分布 `R̂*_iso` ≈ 类别先验 0.49；4/6 评判器的真实 `R̂*_iso` 落在零分布 95% 置信区间外，Falcon 0.47 和 OLMo 0.46 落在内 |
| C12 | 混淆对 UF 金标过滤严格度不变 | [图 16](experiments/P12-uf-consensus-subset/fig_uf_consensus.png) + [stats.json](experiments/P12-uf-consensus-subset/stats.json) | τ ∈ {2,3,4,5}，n 999 → 307：`r(acc, R̂*_iso) ∈ [−0.999, −1.000]`，Fisher-z 置信区间上界 ≤ −0.988 |
| C13 | 第 1 年必须击败的 baseline：朴素众包聚合失败 | [图 17](experiments/P13-consensus-baseline/fig_consensus.png) + [stats.json](experiments/P13-consensus-baseline/stats.json) | HS2 median-of-c = 0.445，UF = 0.299；drift 0.146 = 单评判器 spread 同量级；中位数无法消除系统性容量偏差——从而确立了聚合器下界 |
| C14 | FCI-Risk 部分通过 + 建设性反例 | [图 18](experiments/P15-fci-risk/fig_fci.png) + [stats.json](experiments/P15-fci-risk/stats.json) | UF：跨评判器标准差 0.128 → 0.073（比值 0.57，−43%）；漂移 0.134 → 0.114（−14%）。HS2：标准差 0.031 → 0.061（比值 1.95），因为只有 OLMo 拉伸因子 > 1。整体未通过（drift > 0.09），但 UF 的成功 + HS2 反例精确定位了第 1 年必须填补的稳健性缺口 |

---

## 下一步（写作阶段，2026-04-19 → 5.15）

- 📝 中文版研究计划书初稿 ≥2500 字，deadline 2026-05-03
- 📝 英文终稿 + LaTeX 排版，deadline 2026-05-15
- 📝 M5 逐评判器失败模式（per-judge failure-mode）一段 + C14 partial-PASS framing 折入研究计划书第 1 节
