...
评估 RLHF 偏好数据本质上是一个诊断问题：我们观察到的分歧里，有多少属于*不可约的标注员噪声*，又有多少属于*可约的分类器误差*？**Ishida 等人** (ICLR 2023 oral) 给出了一个无需实例的贝叶斯错误率估计器 `R̂* = (1/N) Σ min(c_i, 1−c_i)`，它从软标签出发——正是我们需要的诊断工具。但在 RLHF 中，软标签来自 **LLM 评判器**，而这些评判器系统性地存在校准偏差 ([Guo 2017](https://arxiv.org/abs/1706.04599))；直接代入得到的估计被评判器**容量**主导，而不是数据侧噪声。[Ushio et al. ICLR 2026] 提出的校准方案——在被污染软标签上做保序回归——在多评判器场景下仍然保留了这种混淆 (C8)。偏好这一侧源自 Bradley–Terry (1952) / Plackett-Luce；现代 DPO ([Rafailov 2023](https://arxiv.org/abs/2305.18290)) 继承了它的噪声假设。Yin & Ishida (ICLR 2026) 的分区式可扩展监督框架，为「专家领域分区」这一介于单实例与整体诊断之间的中间粒度提供了合适的语言。
（作者背景：硕士毕设做的是类别不平衡下的不规则时序分类，使用 Neural LNSDE + 连续时间 Transformer，在 MACHO / LINEAR / ASAS / PhysioNet 上实验。可迁移的角度是对*不平衡、不规则观测数据上的噪声稳健诊断*的经验——而这正是平局密集、标注员分区的 RLHF 场景所需要的。我们**不**主张任何形式化的同构关系。）本仓库是前期证据，用以说明混淆是真实存在的、分区结构可恢复、且**校准感知偏好贝叶斯错误率估计器 `R̂*_CA`** 是可行的第 1 年理论目标。
